  ...
  try {
    ...
    return (
      <div className="flex flex-1 flex-col gap-6 px-4 py-6 lg:px-6">
        <div className="text-center py-12 text-muted-foreground">{t('statsLoadError')}</div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-1 flex-col gap-6 px-4 py-6 lg:px-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-1">{t('statsSubtitle')}</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-sm text-muted-foreground">{t('kpiPublished')}</p>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-sm text-muted-foreground">{t('kpiDrafts')}</p>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-sm text-muted-foreground">{t('kpiScheduled')}</p>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-sm text-muted-foreground">{t('kpiTotalViews')}</p>
      ...
      {stats.mostViewed && (
        <Card className="border-l-4 border-l-green-600">
          <CardHeader className="pb-3">
            <CardTitle className="text-base">📈 {t('mostViewedTitle')}</CardTitle>
          </CardHeader>
          <CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>{t('categoryCount', { n: stats.byCategory.length })}</CardDescription>
        </CardHeader>
        <CardContent>
            ...
            {stats.byCategory.map((cat) => (
              <div key={cat.category} className="flex items-center justify-between">
                <div className="flex-1">
                  ...
                  <p className="text-xs text-muted-foreground">{t('articleCount', { n: cat.count })}</p>
                </div>
                <div className="text-right">
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <div>
              <p className="text-muted-foreground">{t('summaryAvgViews')}</p>
              <p className="text-xl font-bold mt-1">{stats.avgViews}</p>
            </div>
            <div>
              <p className="text-muted-foreground">{t('summaryPublishRate')}</p>
              <p className="text-xl font-bold mt-1">{stats.total > 0 ? Math.round((stats.published / stats.total) * 100) : 0}%</p>
            </div>
            <div>
              <p className="text-muted-foreground">{t('summaryCategories')}</p>
              <p className="text-xl font-bold mt-1">{stats.byCategory.length}</p>
            </div>
            <div>
              <p className="text-muted-foreground">{t('summaryArchived')}</p>
