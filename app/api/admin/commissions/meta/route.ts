...
export async function GET() {
  ...
  const lastUpdated = await getSetting(SETTING_KEYS.COMMISSIONS_LAST_UPDATED);
...
export async function PATCH(req: NextRequest) {
  ...
  await setSetting(SETTING_KEYS.COMMISSIONS_LAST_UPDATED, lastUpdated, authCheck.user.id);
  await logActivity(
    authCheck.user.name,
    "updated",
    "setting",
    "Date de mise à jour des commissions paritaires",
    SETTING_KEYS.COMMISSIONS_LAST_UPDATED,
    `→ ${lastUpdated}`
  );
