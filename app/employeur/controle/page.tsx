...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    title: t("controlMetaTitle"),
    description: t("controlMetaDesc"),
  };
}
  ...
  return (
    <div className="w-full space-y-4 p-4 sm:p-6 lg:px-8 duration-500 animate-in fade-in">
      ...
      <header className="space-y-1">
        <h1 className="text-2xl font-semibold tracking-tight">{t("controlTitle")}</h1>
        <p className="text-muted-foreground">{t("controlIntro")}</p>
      </header>
      <PayslipControl />
    </div>
  );
