...
export function OnemAssignmentsManager() {
  ...
  useEffect(() => {
    let cancelled = false;
    Promise.all([
      fetch("/api/admin/bureaus/onem-assignments").then((r) => r.json()),
  ...
  async function autoAssign(scope: { province?: string; region?: string }, mode: "merge" | "replace") {
    ...
    setSaving(true);
    try {
      const res = await fetch("/api/admin/bureaus/onem-assignments/auto-by-province", {
        method: "POST",
  ...
  async function save() {
    ...
    setSaving(true);
    try {
      const res = await fetch("/api/admin/bureaus/onem-assignments", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
