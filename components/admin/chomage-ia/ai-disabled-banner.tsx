...
export function AiDisabledBanner({ enabled, hasKey }: AiDisabledBannerProps) {
  if (enabled && hasKey) return null;

  const reason: string[] = [];
  if (!enabled) reason.push("le toggle « Aide IA » est désactivé");
  if (!hasKey) reason.push("la variable ANTHROPIC_API_KEY n'est pas configurée");
    ...
    <div className="flex flex-wrap items-start justify-between gap-3 rounded-xl border border-amber-300/60 bg-amber-50/40 p-4 text-amber-900 dark:border-amber-500/30 dark:bg-amber-950/20 dark:text-amber-200">
      <div className="flex items-start gap-2.5">
        ...
        <div className="flex flex-col gap-1">
          <div className="text-[13px] font-bold">L&apos;IA est désactivée</div>
          <p className="text-[12.5px] leading-relaxed">
            Le chat et le générateur de prompts ne pourront pas répondre car{" "}
            {reason.join(" et ")}. La gestion des sources reste possible.
          </p>
        </div>
      </div>
