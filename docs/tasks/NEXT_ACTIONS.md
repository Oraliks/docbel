...
| 22 | P3 | RioLex | Peupler les mappings article↔code Lookup (encart « Codes ONEM liés »). **Série sanctions faite : art. 153/154/155** (156 abrogé ; 157-159 = procédure, aucun code ONEM). Reste : autres tables (admissibilité `129&x` → art. 129, indemnisation `S04/S36`…) — **domaine Oraliks, ne pas inventer** | `lib/data/riolex-lookup-refs.json` | Faible | `pnpm attach:lookup-refs --dry` + fiche article | en cours |
