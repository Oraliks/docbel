...
| 22 | P3 | RioLex | Mappings « Codes ONEM liés » : **89 articles peuplés** (sanction/admissibilité/indemnisation) via `scripts/generate-lookup-refs.ts` (pont code#→article#, abrogés + pseudo-articles méta exclus, variantes Y/Z écartées). **Reste : QA visuelle Oraliks** + affiner libellés/sélection si besoin ; art. 32/129 (base) abrogés donc non mappés | `lib/data/riolex-lookup-refs.json`, `scripts/generate-lookup-refs.ts` | Faible | `pnpm attach:lookup-refs --dry` + fiches article | en cours |
