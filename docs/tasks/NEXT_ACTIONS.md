...
| 46 | P2 | Dette | **Autocomplete employeur (BCE) livre sur C1A/C1B — 2 lancements restent a faire.** (1) `pnpm tsx scripts/apply-c1-improvements.ts --yes` contre la base partagee, sinon `help`/`enterpriseAutocomplete` n'atteignent jamais l'ecran (le runner lit la base, pas le code). (2) Verifie en dev le 2026-08-03 : `GET /api/lookup/entreprise?q=...` repond `{results:[]}` pour des noms d'entreprises connues (Colruyt, Belgacom) — la route et le mirroir sont sains (health check DB OK, aucune erreur), mais le mirroir KBO local semble vide ou tres partiel dans cet environnement. A verifier : `/api/cron/kbo-refresh` a-t-il deja tourne / l'import initial KBO a-t-il ete fait sur cette base ? Sans donnees, l'autocomplete degrade proprement en texte libre (aucun bug), mais n'aide jamais personne | `lib/pdf-forms/seed/c1a-fields.ts`, `c1b-fields.ts`, `lib/be-companies/kbo-etl.ts` | Faible | `curl /api/lookup/entreprise?q=<nom connu>` + ecran C1A/C1B | **Oraliks** — re-semis + verif donnees KBO |
