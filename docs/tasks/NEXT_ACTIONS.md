...
| 33 | P2 | Contenu | **Transcrire la legende « Explications relatives a la rubrique I »** (page 2 de l'Annexe Regis : familles de codes N / A / FN1-FN5 / FY1-FY5) dans la base chomage. Aujourd'hui l'Annexe Regis n'existe NULLE PART dans `docs/knowledge/chomage/` : tout texte d'aide de son seed est structurellement non sourçable. **Confirmé par `/verif-reglementation` le 2026-08-04** (légende complète lue directement sur `private/pdfs/Annexe_Regis_FR.pdf` p.2, faute de base à consulter) : le texte réel du code FN4 est « … je ne règle cependant pas les questions ménagères principalement en commun avec cette personne… » — aucune condition de lien de parenté nulle part dans la légende (N/A/FN/FY). `FN4_HELP` (`lib/pdf-forms/seed/c1-regis-fields.ts:123-124`) dit « (aucun lien de parenté) » : mention sans base textuelle, à retirer ou reformuler sur le seul vrai critère (gestion du ménage non partagée) — un citoyen ayant un lien de parenté mais ne gérant réellement pas le ménage en commun pourrait à tort écarter FN4 sur la foi de cette aide | `docs/knowledge/chomage/formulaires-onem.md`, `RULES_INDEX.md`, `lib/pdf-forms/seed/c1-regis-fields.ts:123-124` | Faible | `/verif-reglementation` sur le seed c1-regis | **Oraliks** — légende en main, correction FN4_HELP à trancher |
...
| 39 | P2 | Metier | **Les 16 declarations de la rubrique IV du C1 arrivent pre-repondues « non ».** Le moule partage `ouiNon` pose `defaultValue: "non"` : un citoyen qui traverse l'etape « Activites & revenus » sans rien toucher signe seize declarations negatives qu'il n'a pas lues, sous une affirmation « sur l'honneur ». C'est peut-etre voulu (c'est la reponse de l'immense majorite, et une case non cochee vaut « non » sur le papier) — mais c'est une decision, pas un defaut technique | `lib/pdf-forms/seed/_shared/moules.ts` (`ouiNon`) | Moyen (ce qui est declare) | parcours C1 etape 3 | **Oraliks** — decision metier |
| 40 | P2 | Contenu | **Le C1-PARTENAIRE se signe A DEUX, et le parcours ne le dit nulle part.** Le papier porte une colonne « date signature du partenaire » et un bandeau « A COMPLETER PAR LE CHOMEUR ET LE PARTENAIRE EN 3 EXEMPLAIRES ». Docbel appose la signature du seul citoyen, puis lui rend le PDF sans lui dire que son partenaire doit signer, ni en combien d'exemplaires envoyer. L'aide de section retombe sur le texte generique « Pourquoi ces questions ? » | `lib/pdf-forms/section-help.ts` | Faible | ecran C1-Partenaire | **FAIT** (feuille de route, 2026-08-02) — l'etape « Envoyer » de l'ecran de sortie liste qui signe et les 3 exemplaires (registre sourcé du bandeau imprimé), et la page de garde du zip/mail le répète sur papier. Le texte reste a valider par Oraliks (review du lot) |
| 41 | P3 | Metier | **NISS a date de naissance future : faut-il aussi le REFUSER a la saisie ?** La derivation ne rend plus de date impossible (corrige le 2026-08-02), mais `diagnoseNISS` accepte toujours un numero dont la branche 20xx donne une naissance a venir — il ne peut pas avoir ete delivre. Le refuser dirait au citoyen « verifiez votre numero » au lieu de le laisser avec une case vide | `lib/pdf-forms/validators.ts` | Moyen (touche inscription, dossiers, tous les formulaires) | `niss-birthdate-futur.test.ts` + parcours | **Oraliks** — decision metier |
| 42 | P3 | Contenu | **Tutoiement restant dans les questionnaires d'orientation** (`lib/dossiers/prepension`, `chomage-frontalier`, `chomage-complet` : « ta situation », « si tu as moins de 60 ans »…). Les seeds PDF sont tous au vouvoiement depuis S10 ; ces trois-la ne l'etaient pas dans le perimetre. Une passe par dossier | `lib/dossiers/*/index.ts` | Faible | relecture ecran | a faire |
| 43 | P3 | Dette | **Le mois de cotisation syndicale du C1 s'imprime toujours sur la PREMIERE ligne.** Les deux lignes « a partir du mois de chomage de » partagent UN champ AcroForm a deux widgets : cocher « je n'autorise plus » et dater donne un papier ou la date figure sur la ligne « j'autorise », qui n'est pas cochee. Latent aujourd'hui (les trois champs sont `readOnly` et laisses vides — decision du 2026-07-26), a traiter le jour ou Docbel remplira cette rubrique. Correctif : deux stamps positionnels + deux regles, comme les en-tetes de page 2 | `lib/pdf-forms/seed/c1/paiement.ts`, `bindings/per-form/c1-changement.ts`, `filler.ts` | Faible (latent) | scenario 4 de `gen-c1-scenarios.ts` | a faire |
| 44 | P3 | Dette | **`niss_partenaire` (C1-PARTENAIRE) s'ecrit d'un bloc sur son peigne de 11 cases** : « 82.06.15-201.46 » deborde sur les premieres cases et laisse les dernieres vides. L'exemption est assumee (`SANS_PEIGNE_ASSUME`) parce que la case accepte « NISS OU date de naissance », qu'un peigne 6-3-2 eparpillerait. Piste : ne peigner que si le nombre de caracteres correspond au nombre de cases — le NISS tomberait dans la grille, la date resterait ecrite en clair. Touche le peigne de tous les documents, donc a faire seul | `lib/pdf-forms/filler.ts`, `seed/c1-partenaire-fields.ts` | Moyen (change ce qui s'imprime partout) | les 8 recettes + relecture | a faire |
| 45 | P3 | Contenu | **Trois documents impriment le nom d'en-tete dans deux ordres.** Sous le meme libelle imprime « Nom », le C1 ecrit « Prenom Nom » quand le C1A et le C1B ecrivent « Nom Prenom ». Aucun n'est faux ; l'incoherence se voit quand les trois pages arrivent ensemble a l'ONEM | `lib/pdf-forms/filler.ts` (regle `date-header-p2` / stamps positionnels), `bindings/per-form/c1a.ts`, `c1b.ts` | Faible | PDF genere des trois | **FAIT** (2026-08-03) — tout le parc ecrit « Nom Prenom », l'ordre du libelle imprime et celui de la page 1 du C1 |
| 46 | P2 | Dette | **Autocomplete employeur (BCE) livre sur C1A/C1B — 2 lancements restent a faire.** (1) `pnpm tsx scripts/apply-c1-improvements.ts --yes` contre la base partagee, sinon `help`/`enterpriseAutocomplete` n'atteignent jamais l'ecran (le runner lit la base, pas le code). (2) **Diagnostic confirme le 2026-08-04** : mirroir KBO **vide (0 ligne)** dans les 4 tables (`KboEnterprise`/`Denomination`/`Address`/`Activity`) de la base partagee, et `KBO_OPEN_DATA_USER`/`PASSWORD` absents de `.env` et `.env.local` (le cron `/api/cron/kbo-refresh` est donc un no-op silencieux depuis toujours, pas juste "pas encore passe"). Il faut : un compte KBO Open Data (SPF Economie) pour les identifiants, PUIS un premier import manuel non-serverless via `/api/admin/lookup/bce/refresh` (l'import complet ~2M entreprises depasse le timeout d'une fonction serverless, cf. commentaire `kbo-refresh/route.ts`). Sans donnees, l'autocomplete degrade proprement en texte libre (aucun bug), mais n'aide personne | `lib/pdf-forms/seed/c1a-fields.ts`, `c1b-fields.ts`, `lib/be-companies/kbo-etl.ts`, `.env.local` | Faible (code) / depend d'un compte externe | `pnpm tsx` count KboEnterprise + `curl /api/lookup/entreprise?q=<nom connu>` | **Oraliks** — re-semis + compte KBO Open Data + 1er import manuel |
| 46 | P3 | Contenu | **L'ecran de SIGNATURE affiche « Pourquoi ces questions ? »** sur les sept compagnons — le repli generique de `section-help.ts`, alors qu'il ne reste aucune question a poser : le citoyen y appose une declaration sur l'honneur. Une entree `signature` dediee (« votre signature est apposee automatiquement, relisez vos reponses ») couvrirait les huit documents d'un coup. Sept autres cles retombent aussi sur le generique (`employeur`, `activites-anterieures`, `grille-differences`, `mandat-culturel`, `partenaire`) — moins genant, meme fichier | `lib/pdf-forms/section-help.ts` | Faible | ecran de signature d'un document | **FAIT** (2026-08-03) — 6 entrees ecrites (fr/nl/de) ; la signature ne menant JAMAIS une etape, `choisirCleDAide` lui donne la priorite sur les autres sections de son ecran |
| 47 | P3 | Dette | **Le C1 imprime « oui → allez à la rubrique suivante » sur `statutRefugie`/`apatrideReconnu`, mais le runner pose quand même la question d'accès au marché du travail ensuite** (`accesMarcheTravail` ne dépend que de `nationaliteHorsEEE`). Second constat lié : le papier pose en réalité TROIS questions successives dans ce bloc (réfugié / apatride / **« je dispose d'un document de séjour »**), et cette 3e question n'a AUCUN champ dans le seed — `accesMarcheTravail` s'affiche directement sans ce palier, et son option « non » ne distingue pas « j'ai un document mais limité » de « aucun document ». Repéré en documentant l'aide de champ (2026-08-04, audit `/verif-reglementation` à l'appui) — rien corrigé dans ce lot de contenu | `lib/pdf-forms/seed/c1/final.ts` | Faible (pas de case blanche, juste un palier de questions simplifié) | parcours C1 étape hors-EEE + relecture `private/pdfs/C1_FR.pdf` p.2 | **Oraliks** — décider si le palier « document de séjour » doit être ajouté et le saut implémenté |
