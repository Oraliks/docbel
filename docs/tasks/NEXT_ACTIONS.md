...
| 23 | P3 | Design | Unifier l'idiome de survol `hover:bg-white/NN` → `hover:bg-[color:var(--glass-surface)]` **repo-wide** (rendu mixte après le sweep couleurs du 2026-07-22 : converti sur certains écrans, laissé sur d'autres ; garder les voiles sur panneaux colorés) | ~9 fichiers front (login, formations, glossaire, tool-page, mot-de-passe-oublie…) | Faible | build + hover clair/sombre | à faire |
| 24 | P3 | Design | Migrer le vocabulaire de **dégradés décoratifs partagé** (`VARIANT_BG`, `THEME` employeur, tuiles rainbow) vers tokens — **dédié codebase-wide** (s'affiche sur l'accueil ; cf. DESIGN_RULES « incohérences connues ») | `components/docbel/landing/tool-card.tsx`, `p/*`, `confirm-account.tsx` | Moyen | écrans accueil + `/p` clair/sombre | à faire |
| 25 | P3 | Data | Harmoniser les couleurs **stockées en DB** (`bundle.color` — dont un `#7C3AED` périmé visible sur mon-dossier —, `News.color`, accent d'outil) vers la palette système ; **QA dark des teintes `--chart-*`** (permutées en sombre par design) sur les usages décoratifs du sweep 2026-07-22 | seeds/DB (SQL additif), `scripts/*` | Faible | mon-dossier + écrans sombre | à faire |
