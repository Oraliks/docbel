...
## Unification « Parcours & dossiers » — Lots 1+2 LIVRÉS (branche `feat/parcours-dossiers-unification`, poussée)
Rapproche PDF Forms ↔ Decision Builder (déjà couplés : moteur de conditions
partagé, arbre → `bundleSlug` → run). Commits `2b906e3`→`789be78`, 1390 tests verts,
`/admin` compile (HTTP 404 = garde auth, pas d'erreur).
...
- **Restant** : **Lot 3** — funnel unique (recherches → orientation → run → complété →
  téléchargements) en fusionnant les 2 dashboards analytics (données déjà là :
  BundleAnalyticsEvent + analytics arbres + PdfFormSubmissionLog). **Lot 4** (métier,
  décision Oraliks) — vocabulaire de clés de réponses canoniques partagé entre options
  d'arbre et `eligibilityQuestions` (dédupliquer orientation ↔ pré-qualif, préremplir).
