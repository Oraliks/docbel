...
| 22 | P3 | RioLex | Peupler les mappings article↔code Lookup (encart « Codes ONEM liés » livré, vide sans données) : renseigner `lib/data/riolex-lookup-refs.json` puis `pnpm attach:lookup-refs` — **domaine Oraliks, ne pas inventer** | `lib/data/riolex-lookup-refs.json` | Faible | `pnpm attach:lookup-refs --dry` + fiche article | à faire |
