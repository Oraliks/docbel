 ...
 * Accepte un planning unique OU une liste (collage multi-jours) → **une page A4
 * par journée**, chacune avec le titre, la date et la couleur de son jour.
 */
export async function renderPlanningPdf(
  input: Planning | Planning[],
): Promise<Blob> {
  const plannings = Array.isArray(input) ? input : [input];
  ...
  /** Dessine un planning (une journée) sur la page courante du document. */
  function drawPlanning(planning: Planning): void {
    ...
    doc.text(`${planning.total} rendez-vous`, tableX + tableW, y + 5, {
      align: "right",
    });
  }

  plannings.forEach((planning, i) => {
    if (i > 0) doc.addPage();
    drawPlanning(planning);
  });
